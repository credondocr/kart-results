...
export const generateGeneralLeaderboard = (leaderboard: {
  ...
  const combinedClasses: Record<string, Class> = {};
  const availableSeasons = [leaderboard.invierno, leaderboard.verano].filter(Boolean); // Filtramos las temporadas existentes
  ...
  const referenceYear = availableSeasons.length > 0 ? availableSeasons[0]!.year : new Date().getFullYear();
